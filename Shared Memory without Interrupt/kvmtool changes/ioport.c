...
static bool dummy_ioport_ops_io_in(struct ioport *ioport, struct kvm_cpu *vcpu, u16 port, void *data, int size)
{
	int success;
	
	FILE *fd=fopen("/proc/shmem_comm_VM","r");
...
static bool dummy_ioport_ops_io_out(struct ioport *ioport, struct kvm_cpu *vcpu, u16 port, void *data, int size)
{
	u8 ch;
	ch = ioport__read8(data);
	char temp_ch = 65 + ch;
	...
	strcpy(command,"echo ");
	command[5] = temp_ch;	
	strcat(command," > /proc/shmem_comm_VM");
...
static struct ioport_operations dummy_ioport_ops = {
	.io_in		= dummy_ioport_ops_io_in,
	.io_out		= dummy_ioport_ops_io_out, 
};
...
void ioport__setup_arch(struct kvm *kvm)
{
	...
	ioport__register(kvm, 0x0092, &dummy_ioport_ops, 2, NULL);
