...
static irqreturn_t ivshmem_interrupt(int irq, void *dev_id)
{
	...
	//readl(ivshmem_dev.regs_base_addr + IntrStatus); //status =
			 ...
			 const struct pci_device_id *pdev_id)
{
	...
	ivshmem_dev.regs_start = pci_resource_start(pdev, 1);
	ivshmem_dev.regs_len = pci_resource_len(pdev, 1);
	ivshmem_dev.regs_base_addr =pci_iomap(pdev, 1, 0); 
