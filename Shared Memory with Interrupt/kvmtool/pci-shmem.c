...
//#include "kvm/generate_eventfd.h"
...
int pci_shmem__register_mem(struct shmem_info *si)
{
	if (!shmem_region) {
		shmem_region = si;
		pr_info("shmem: ownership takeover complete");
	} else {
...
static void callback_mmio_msix(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len, u8 is_write, void *ptr)
{
	...
	if (is_write){		
		...
		pr_warning("written to..\n");
		
		if(data_intr == 0){	
			pr_warning("doing cat..\n");
			
			int success;	
			FILE *fd=fopen("/proc/bridge_comm_VM","r");	
			success = fgetc(fd); 	//cat /proc/shmem_comm_VM			
			if(success < 0)
				perror("exec error: \n");
			fclose(fd);
		}
		else if(data_intr == 1){
			pr_warning("doing echo..\n");
			
			const char *command = "echo A > /proc/bridge_comm_VM";		
			
			if(system(command) > 0)
				perror("exec error \n");
		
			uint64_t value = 1;
			int i = write (local_VM_fd, &value, sizeof (value)); //interrupt to writer: its safe to write!
			pr_warning("eventfd sent back to VM: %d \n", i);
		}
...
int pci_shmem__init(struct kvm *kvm)
{
	...
	r = ioport__register(kvm, IOPORT_EMPTY, &shmem_pci__io_ops, IOPORT_SIZE, NULL); /* IOPORT_EMPTY 0xFFFFU */
	if (r < 0)
		return r;
...
/************************************************/	
	local_VM_fd = pci_shmem__get_local_irqfd(kvm);
	pr_info("Local eventfd for VM : %d \n", local_VM_fd);	
